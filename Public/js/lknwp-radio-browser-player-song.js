document.addEventListener('DOMContentLoaded', function () {
    ...
    if (window.location.protocol === 'https:' && streamUrl && streamUrl.startsWith('http:')) {
        ...
        warningDiv.innerHTML = 'Atenção: Esta rádio utiliza streaming inseguro (HTTP) e não pode ser reproduzida em páginas HTTPS. Solicite ao provedor que habilite HTTPS ou acesse por HTTP.';
        ...
        if (musica) {
            ...
            if (window.LKNWP_STATION_CLICKCOUNT > 0 || window.LKNWP_STATION_VOTES > 0) {
                ...
                if (window.LKNWP_STATION_CLICKCOUNT > 0) {
                    statsText = formatNumber(window.LKNWP_STATION_CLICKCOUNT) + ' ouvintes';
                } else if (window.LKNWP_STATION_VOTES > 0) {
                    statsText = formatNumber(window.LKNWP_STATION_VOTES) + ' likes';
                }
                ...
                fetchWithTimeout(methodUrl, 10000)
                    ...
                    .then(function (result) {
                        ...
                        if (isAudioComponent(result.response, html)) {
                            throw new Error('Método otimizado falhou - resposta é componente de áudio');
                        }
                        var musicaHtml = '';
                        var artistaHtml = '';
                        var match = null;
                ...
                fetchWithTimeout(method.url, 10000)
                    .then(function (response) {
                        ...
                        if (response.type === 'opaque') {
                            throw new Error('CORS_BLOCKED: Resposta opaca, não é possível ler o conteúdo');
                        }

                        // Se status é 0 ou não está ok, também pode ser problema de CORS
                        if (!response.ok && response.status === 0) {
                            throw new Error('NETWORK_ERROR: Status 0, possível problema de rede ou CORS');
                        }
                        ...
                        if (contentType.includes('audio/') || contentType.includes('application/octet-stream')) {
                            throw new Error('AUDIO_STREAM: Resposta é um stream de áudio');
                        }
                        ...
                        return Promise.race([
                            ...
                            new Promise(function (_, reject) {
                                setTimeout(function () {
                                    reject(new Error('TEXT_TIMEOUT: Conversão de texto excedeu 5 segundos'));
                                }, 5000);
                            })
                        ]);
                    })
    ...
    function tryParseJsonData(result) {
        try {
            ...
            if (!contentType.includes('application/json')) {
                return { found: false, reason: 'Content-Type não é JSON: ' + contentType };
            }

            if (isAudioComponent(result.response, result.text)) {
                return { found: false, reason: 'Detectado componente de áudio' };
            }
            ...
            if (song && song.trim().length > 0) {
                ...
                return { found: false, reason: 'Nenhuma música encontrada no JSON' };
            }
        } catch (e) {
    ...
    function tryParseHtmlData(result) {
        ...
        if (match && match[1] && match[1].trim()) {
            ...
            return { found: false, reason: 'Nenhuma música encontrada no HTML' };
        }
    }
